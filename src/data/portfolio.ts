...
L'architecture repose sur plus de 14 microservices NestJS (authentification, CRM, calendrier, interventions, stocks, finance, notifications, messagerie…), orchestrés par un API Gateway. Chaque service possède sa propre base PostgreSQL via Prisma ORM, et communique par événements Redis Pub/Sub. Le frontend comprend trois portails distincts — administration, agents terrain et clients — tous développés en Next.js avec Tailwind CSS. L'ensemble est conteneurisé avec Docker et déployé sur un serveur OVH avec Nginx, SSL Let's Encrypt et monitoring complet.`,
