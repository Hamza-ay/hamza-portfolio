        ...
        <div className="grid md:grid-cols-2 gap-6 sm:gap-8">
          ...
          <ScrollReveal variant="slideLeft" delay={0.1}>
            <GlassCard className="p-8 h-full">
              ...
              <div className="text-mist-400 leading-relaxed mb-6 space-y-4">
                {personalInfo.aboutDescription.split("\n\n").map((paragraph, idx) => (
                  <p key={idx}>{paragraph}</p>
                ))}
              </div>
              <div className="flex flex-wrap gap-4 text-sm text-mist-400">
              ...
              <div className="mt-6 pt-6 border-t border-white/[0.06]">
                <h4 className="font-mono text-xs text-mist-500 mb-3">Langues</h4>
                <div className="flex flex-wrap gap-3">
                  {languages.map((lang) => (
                    <span
                      key={lang.name}
                      className="px-3 py-1.5 rounded-full bg-white/[0.05] border border-white/[0.08] text-sm text-mist-300"
                    >
              ...
              {/* Interests */}
              <div className="mt-6 pt-6 border-t border-white/[0.06]">
                <h4 className="font-mono text-xs text-mist-500 mb-3">Centres d&apos;intérêt</h4>
                <div className="flex flex-wrap gap-3">
                  {interests.map((i) => (
                    <span
                      key={i.name}
            ...
            <GlassCard className="p-8 h-full">
              ...
              <div className="space-y-6">
                {education.map((edu, idx) => (
                  <div key={idx} className="relative pl-6 border-l-2 border-accent-500/30">
                    <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-accent-500/20 border-2 border-accent-500" />
                    <span className="text-xs font-mono text-accent-400">{edu.period}</span>
                    <h4 className="text-base font-semibold text-white mt-1">{edu.title}</h4>
                    <p className="text-sm text-mist-400 mt-0.5">{edu.school}</p>
                    <p className="text-xs text-mist-500 mt-1">{edu.details}</p>
                  </div>
                ))}
              </div>
            </GlassCard>
