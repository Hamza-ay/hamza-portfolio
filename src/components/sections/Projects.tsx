...
function ProjectCard({ project }: { project: (typeof projects)[number] }) {
  ...
  return (
    <div className="p-6 bg-white/[0.03] backdrop-blur-sm border border-white/[0.06] rounded-2xl hover:border-indigo-500/30 transition-all duration-300 h-full group flex flex-col">
      {/* Project image */}
      ...
      <p className={`text-sm text-zinc-400 leading-relaxed mb-4 ${!expanded ? "line-clamp-2" : ""}`}>
        {project.description}
      </p>
        ...
        {expanded && hasDetails && (
          ...
          >
            <div className="pb-4 pt-1 border-t border-white/[0.06] mt-1">
              <div className="text-sm text-zinc-400 leading-relaxed space-y-3 mt-3">
      ...
      {hasDetails && (
        <button
          ...
          className="flex items-center gap-1.5 text-xs text-indigo-400 hover:text-indigo-300 transition-colors mb-4 group/btn"
        >
          <motion.span
